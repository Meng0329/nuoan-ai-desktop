...
// 全局Axios优先使用IPv4，避免某些网络环境下IPv6解析失败
try { axios.defaults.family = 4; } catch (_) {}
...
if (isDev) {
  ...
  console.log('[Desktop] 🔧 开发模式：已禁用SSL证书验证');
}

// 后端API配置
// 调整优先级：环境变量 > 持久化配置 > 打包后默认线上域名 > 本地开发
function sanitizeApiBaseUrl(raw) {
  try {
...
// 开发模式下强制使用本地API，避免使用缓存的线上地址
let apiBaseUrl;
if (isDev) {
  // 开发模式：优先使用环境变量；其次使用已保存配置；默认改为 HTTPS 线上地址
  const devCandidate = process.env.NUOAN_API_BASE_URL
    || store.get('apiBaseUrl')
    || 'https://nuoanai.com/api';
  apiBaseUrl = sanitizeApiBaseUrl(devCandidate);
  console.log('[Desktop] 🔧 开发模式：使用API地址:', apiBaseUrl);
} else {
  // 生产模式：按优先级读取配置
  apiBaseUrl = sanitizeApiBaseUrl(
    process.env.NUOAN_API_BASE_URL
      || store.get('apiBaseUrl')
      || 'https://nuoanai.com/api'
  );
}
try { console.log('[Desktop] 使用后端API地址:', apiBaseUrl); } catch (_) {}

async function quickHealthcheck(baseUrl) {
  try {
    ...
    for (const u of urls) {
      try {
        const resp = await axios.get(u, { timeout: 5000 });
...
let autoAuthInterval = null; // 新增：自动认证定时器

// 网络连接检测 - 已禁用
async function checkNetworkConnection() {
  // 直接返回 true，不再进行实际网络检测
  return true;
}

// 检测后端服务可用性 - 已禁用
async function checkBackendAvailability() {
  // 直接返回 true，不再进行实际后端检测
  return true;
}
...
async function getHardwareId(forceRecompute = false) {
  try {
    ...
    const [system, baseboard, cpu, osInfo, diskLayout, bios] = await Promise.all([
      si.system(),
      si.baseboard(),
      si.cpu(),
      si.osInfo(),
...
async function authenticateWithBackend(retryCount = 0) {
  ...
  try {
    ...
    // 记录调试信息
    try {
      const fullUrl = `${apiBaseUrl}/desktop/authenticate`;
      console.log('[Desktop][Auth] 准备认证:', {
        apiBaseUrl,
        fullUrl,
        uid: hardwareId,
        hasPrevUid: !!prevHardwareId,
        deviceInfo,
        retryCount,
      });
      console.log('[Desktop][Auth] 完整请求URL:', fullUrl);
    } catch (_) {}
    ...
    if (response.data.success) {
      ...
      setImmediate(async () => {
        try {
          if (responseData.device?.requireUidRefresh) {
      ...
      setImmediate(async () => {
        try {
          const serverDeviceUid = responseData.device?.uid;
          const freshId = await getHardwareId(true);
    ...
    try {
      ...
      console.error(`设备认证失败 (尝试 ${retryCount + 1}/${maxRetries + 1})`, {
    ...
    // 404 兜底：尝试补齐/还原被反代截掉的 /api 前缀
    if (error?.response?.status === 404) {
      try {
        const baseNoApi = apiBaseUrl.replace(/\/api$/, '');
        const altUrl = `${baseNoApi}/api/desktop/authenticate`;
        const primaryUrl = `${apiBaseUrl}/desktop/authenticate`;
        if (altUrl !== primaryUrl) {
          console.log('[Desktop][Auth] 404兜底重试:', primaryUrl, '=>', altUrl);
          const response = await axios.post(altUrl, {
            uid: hardwareId,
            prevUid: prevHardwareId || undefined,
            deviceInfo
          }, {
        ...
        console.warn('[Desktop][Auth] 404兜底重试失败:', e2?.message || e2);
      }
    }
    ...
    if (error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT' || 
      ...
      // 开发模式下不切换协议，生产模式才切换
      if (!isDev) {
        // 一次性尝试协议切换（https<->http）
        try {
          const u = new URL(apiBaseUrl);
          const toggled = `${u.protocol === 'https:' ? 'http:' : 'https:'}//${u.host}${u.pathname || ''}`.replace(/\/$/, '');
          if (toggled !== apiBaseUrl) {
            console.log('[Desktop][Auth] 协议切换尝试:', apiBaseUrl, '=>', toggled);
            apiBaseUrl = toggled;
...
async function verifyDeviceStatus(retryCount = 0) {
  ...
  try {
    try {
      const fullUrl = `${apiBaseUrl}/desktop/verify-device`;
      console.log('[Desktop][Verify] 验证设备状态:', {
        apiBaseUrl,
        fullUrl,
        uid: hardwareId,
        retryCount,
      });
      console.log('[Desktop][Verify] 完整请求URL:', fullUrl);
    } catch (_) {}
    ...
    try {
      console.error(`设备状态验证失败 (尝试 ${retryCount + 1}/${maxRetries + 1})`, {
        message: error?.message,
        code: error?.code,
        errno: error?.errno,
        syscall: error?.syscall,
    ...
    if (error?.response?.status === 404) {
      try {
        const baseNoApi = apiBaseUrl.replace(/\/api$/, '');
        const altUrl = `${baseNoApi}/api/desktop/verify-device`;
        const primaryUrl = `${apiBaseUrl}/desktop/verify-device`;
        if (altUrl !== primaryUrl) {
          console.log('[Desktop][Verify] 404兜底重试:', primaryUrl, '=>', altUrl);
          const response = await axios.post(altUrl, { uid: hardwareId }, {
            headers: { Authorization: `Bearer ${authToken}` },
            timeout: 15000
          });
          if (response.data.success) {
            console.log('设备状态验证成功(404兜底)');
            return response.data.data;
          }
        }
      } catch (e2) {
        console.warn('[Desktop][Verify] 404兜底重试失败:', e2?.message || e2);
      }
    }
...
// 新增：启动自动认证定时器
function startAutoAuthTimer() {
  // 每10分钟自动认证一次，确保设备信息持续同步到后台
  autoAuthInterval = setInterval(async () => {
    try {
      console.log('定期自动认证：发送设备信息到后台...');
      const result = await authenticateWithBackend();
      if (result) {
        console.log('定期自动认证成功');
      }
    } catch (error) {
      console.log('定期自动认证失败:', error.message);
    }
  }, 10 * 60 * 1000); // 10分钟
}

// 新增：停止自动认证定时器
function stopAutoAuthTimer() {
  if (autoAuthInterval) {
    clearInterval(autoAuthInterval);
    autoAuthInterval = null;
  }
}
...
function createLocalServer() {
  const server = http.createServer((req, res) => {
    ...
    } else if (pathname === '/api/auth-info' && req.method === 'GET') {
      // 新增：返回认证信息（包含token和用户信息），供网页端自动登录
      res.writeHead(200, { 'Content-Type': 'application/json' });
      
      const storedToken = store.get('authToken');
      const storedUserInfo = store.get('userInfo');
      const storedDeviceInfo = store.get('deviceInfo');
...
ipcMain.handle('authenticate', async () => {
  try {
    ...
    startAutoAuthTimer(); // 新增：启动定期自动认证
...
app.whenReady().then(() => {
  ...
  setTimeout(async () => {
    console.log('\n' + '='.repeat(60));
    console.log('🔍 [启动诊断] 测试服务器连接');
    console.log('='.repeat(60));
    console.log('当前API地址:', apiBaseUrl);
    
    // 测试几个关键接口
    const testUrls = [
      { name: 'Health检查', url: `${apiBaseUrl}/health` },
      { name: '桌面版本', url: `${apiBaseUrl}/desktop/version` },
      { name: '桌面下载信息', url: `${apiBaseUrl}/desktop/download-info` },
    ];
    
    for (const test of testUrls) {
      try {
        const response = await axios.get(test.url, { timeout: 3000, validateStatus: () => true });
        const status = response.status;
        const icon = status === 200 ? '✅' : status === 404 ? '❌' : '⚠️';
        console.log(`${icon} ${test.name}: ${status} - ${test.url}`);
        if (response.data?.message) {
          console.log(`   消息: ${response.data.message}`);
        }
      } catch (error) {
        const msg = error.code === 'ECONNREFUSED' ? '连接被拒绝' : 
                   error.code === 'ETIMEDOUT' ? '连接超时' :
                   error.code === 'ENOTFOUND' ? '域名解析失败' :
                   error.message;
        console.log(`❌ ${test.name}: ${msg} - ${test.url}`);
      }
    }
    console.log('='.repeat(60) + '\n');
  }, 500);
  ...
  setTimeout(async () => {
    ...
    // 新增：启动时优先尝试验证，失败再自动认证并发送设备信息到后端
    setTimeout(async () => {
      try {
        // 开发模式下跳过健康检查和协议切换，直接使用配置的地址
        if (!isDev) {
          // 生产模式：先健康检查，快速失败时切换协议一次
          const healthy = await quickHealthcheck(apiBaseUrl);
          if (!healthy) {
            try {
              const u = new URL(apiBaseUrl);
              const toggled = `${u.protocol === 'https:' ? 'http:' : 'https:'}//${u.host}${u.pathname || ''}`.replace(/\/$/, '');
              if (toggled !== apiBaseUrl) {
                console.log('[Desktop][Init] 健康检查失败，协议切换:', apiBaseUrl, '=>', toggled);
                apiBaseUrl = toggled;
                store.set('apiBaseUrl', apiBaseUrl);
              }
            } catch (_) {}
          ...
          console.log('[Desktop] 🔧 开发模式：跳过健康检查和协议切换');
        }

        let result = null;
        if (authToken) {
          console.log('检测到已存在token，先验证状态...');
          const verify = await verifyDeviceStatus();
          if (verify) {
            result = { user: store.get('userInfo'), device: store.get('deviceInfo') };
          }
        }

        if (!result) {
          console.log('开始自动认证并发送设备信息...');
          result = await authenticateWithBackend();
        }
        if (result) {
          console.log('自动认证成功，设备信息已发送到管理后台');
          startVerificationTimer();
          startAutoAuthTimer(); // 启动定期自动认证
          
          // 通知渲染进程自动认证成功
          if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('auto-auth-success', {
              user: result.user,
        ...
        // 即使首次自动认证失败，也启动定期认证定时器，后续会自动重试
        startAutoAuthTimer();
        
        // 通知渲染进程自动认证失败，但不影响程序运行
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.send('auto-auth-failed', {
            error: error.message
          });
        }
      }
...
app.on('window-all-closed', () => {
  ...
  stopAutoAuthTimer(); // 新增：停止自动认证定时器
  if (updateTimer) {
